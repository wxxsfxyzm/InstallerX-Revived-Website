...
作用域列表可以显示无法识别安装发起方的**未知作用域**。当你想按来源区分安装行为时，可以新建配置并设置作用域。例如：让应用商店后台静默安装，让文件管理器始终弹出确认对话框。
...
开启智能授权后，所选授权器不可用时会按顺序尝试启用的后备授权器；至少保留一个候选项。
...
配置还可以开启**自动批准会话安装**，默认关闭，只应对可信来源启用。配置文件中可以配置 Toast 的行为，可以设置为从不显示、始终显示，或仅在对话框不可见时显示。

## 安装器设置

配置中的签名策略为**禁止安装签名不匹配的应用**和**禁止安装签名未知的应用**。流式来源无法执行完整签名验证。这些选项会修改安装元数据或安装后的附加行为。
